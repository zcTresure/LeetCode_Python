...
class Solution:
    def minimalSteps(self, maze: list) -> int:
        ...
        # 起点到任意一点的距离
        startToAnyPos = bfs(start_x, start_y, maze, m, n)
        # 若没有机关，最短距离就是(start_x, start_y)到(end_x, end_y)的距离
        if nb == 0:
            return startToAnyPos[end_x][end_y]
        ...
        for i in range(nb):
            ...
            # 第i个机关到终点的距离就是(bx, by)到(end_x, end_y)的距离
            dist[i][nb + 1] = iToAnyPos[end_x][end_y]
        for i in range(nb):
            # 计算第i个机关到(start_x, start_y)的距离
            # 即从第i个机关出发，通过每个石头(sx, sy)，到(start_x, start_y)的最短距离
            tmp = -1
            for j in range(ns):
                sx, sy = stones[j]
        ...
        # dp数组， -1代表没有遍历到, 1<<nb表示题解中提到的mask, dp[mask][j]表示当前处于第j个机关
        # 总的触发状态为mask所需要的最短路径, 由于有2**nb个状态，因此1<<nb的开销必不可少
